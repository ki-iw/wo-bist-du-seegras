...
def seafeats_binary_model(bag_of_seagrass_binary_fixture):
    return bag_of_seagrass_binary_fixture.get_seafeats(weights_path=SEAFEATS_WEIGHTS)


@pytest.fixture
def seaclips_binary_model(bag_of_seagrass_binary_fixture):
    return bag_of_seagrass_binary_fixture.get_seaclips(weights_path=SEACLIP_WEIGHTS)


@pytest.fixture
def seabag_binary_ensemble(seafeats_binary_model, seaclips_binary_model):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    return SeabagEnsemble(model_1=seafeats_binary_model, model_2=seaclips_binary_model, device=device)

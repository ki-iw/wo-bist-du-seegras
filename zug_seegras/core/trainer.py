...
class Trainer:
    def __init__(
        ...
        self.evaluator = Evaluator(transforms=self.train_loader.dataset.transform, device=self.device)
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, num_epochs):
            ...
            log.info(f"Epoch [{epoch + 1}/{num_epochs}], Loss: {running_loss / len(self.train_loader):.4f}")

            if (epoch + 1) % 5 == 0:
                accuracy, f1_score = self.evaluator.run_evaluation(model=self.model, dataloader=self.test_loader)
                log.info(f"Epoch [{epoch + 1}], Accuracy: {accuracy:.4f}, F1 Score: {f1_score:.4f}")
